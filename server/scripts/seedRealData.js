const MGNREGAAPIClient = require('../utils/mgnregaAPI');
...
// Comprehensive Uttar Pradesh districts data
const UP_DISTRICTS = [
...
class RealDataSeeder {
  constructor() {
    this.apiClient = new MGNREGAAPIClient();
  ...
  async getDatabase() {
    if (this.isProduction && process.env.DATABASE_URL) {
      ...
      const dbPath = path.join(__dirname, '../db/mgnrega_dev.db');
  ...
  async seedMGNREGAData(db) {
    ...
    try {
      ...
      console.log('🌐 Attempting to fetch real MGNREGA data...');
      const realData = await this.apiClient.getDistrictData('Uttar Pradesh', 100);
      
      if (realData.length > 0) {
        console.log(`📥 Fetched ${realData.length} real records from API`);
        await this.insertMGNREGAData(db, realData, true);
      } else {
        console.log('⚠️ No real data available, using sample data');
        const sampleData = await this.apiClient.getSampleData();
        await this.insertMGNREGAData(db, sampleData.districts, false);
      }
    } catch (error) {
  ...
  async insertMGNREGAData(db, data, isRealData) {
    console.log(`💾 Inserting ${data.length} MGNREGA records (${isRealData ? 'real' : 'sample'} data)...`);
    
    // Get district IDs
    let districts;
    if (this.isProduction) {
      const result = await db.query('SELECT id, name FROM districts WHERE state_id = $1', ['UP']);
      districts = result.rows;
    } else {
      districts = await new Promise((resolve, reject) => {
        db.all('SELECT id, name FROM districts WHERE state_id = ?', ['UP'], (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      });
    ...
    for (const record of data) {
      ...
      for (let monthsBack = 0; monthsBack < 24; monthsBack++) {
        ...
        const seasonalFactor = Math.sin((month - 1) * Math.PI / 6) * 0.2 + 1; // Seasonal variation
        
        const monthlyRecord = {
          district_id: districtId,
          year,
          month,
          households_registered: Math.floor(record.households_registered * variation),
          households_work_provided: Math.floor(record.households_work_provided * variation * seasonalFactor),
          total_persondays: Math.floor(record.total_persondays * variation * seasonalFactor),
          wages_paid: Math.floor(record.wages_paid * variation * seasonalFactor),
          women_participation_pct: Math.min(100, record.women_participation * (0.9 + Math.random() * 0.2)),
          works_completed: Math.floor(record.works_completed * variation),
          works_ongoing: Math.floor(record.works_ongoing * variation),
        ...
        if (this.isProduction) {
          ...
          await new Promise((resolve, reject) => {
            ...
            `, [
              ...
              monthlyRecord.total_persondays, monthlyRecord.wages_paid,
              monthlyRecord.women_participation_pct, monthlyRecord.works_completed,
              monthlyRecord.works_ongoing, monthlyRecord.avg_wage, monthlyRecord.source_date.toISOString()
            ], function(err) {
              if (err) reject(err);
              else resolve();
            });
        ...
        insertedRecords.push(monthlyRecord);
      }
    }

    console.log(`✅ Inserted ${insertedRecords.length} monthly records`);
  }

  async run() {
    console.log('🚀 Starting real data seeding...');
    console.log(`📍 Environment: ${this.isProduction ? 'Production (PostgreSQL)' : 'Development (SQLite)'}`);
    
    const db = await this.getDatabase();
    
    try {
      await this.seedDistricts(db);
      await this.seedMGNREGAData(db);
      ...
      if (this.isProduction) {
        const summary = await db.query(`
          SELECT 
            (SELECT COUNT(*) FROM districts WHERE state_id = 'UP') as up_districts_count,
            (SELECT COUNT(*) FROM mgnrega_monthly m JOIN districts d ON m.district_id = d.id WHERE d.state_id = 'UP') as up_monthly_records_count
        `);
